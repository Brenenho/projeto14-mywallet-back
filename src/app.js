...
app.post("/cadastro", async (req, res) => {
    ...
    try {
        ...
        await db.collection("users").insertOne({ nome, email, senha: bcrypt.hashSync(senha, 10) })
...
app.post("/login", async (req, res) => {
    ...
    try {
        ...
        const tokenExists = await db.collection("sessao").findOne({ idUsuario: userExists._id })
...
app.post("/nova-transacao/:tipo", async (req, res) => {
    ...
    try {
        ...
        await db.collection("operacoes").insertOne({ user: user.email ,tipo, valor, descricao, data})
